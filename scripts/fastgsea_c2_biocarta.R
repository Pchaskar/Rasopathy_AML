...
biocarta <- c2BroadSets[c(grep("^B_", names(c2BroadSets)))]
...
plot_gsea<-ggplot(fgseaResTidy_top, aes(reorder(pathway, NES), NES)) +
  ...
  labs(x="Gene Sets", y="Normalized Enrichment Score",
       title="C2: biocarta gene sets NES from fgsea") + 
  theme_classic()

svg("./images/fgsea_c2_biocarta.svg", width = 10, height = 10)
plot_gsea
dev.off()
...
write.xlsx(fgseaResTidy,row.names = TRUE, file = paste("./gsea/c2_biocarta","fgsea.xlsx", sep="_"))
